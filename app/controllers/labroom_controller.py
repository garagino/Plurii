...
class LabRoomController:
    ...
    def create_labroom(db: Session, room: LabRoom):
        db_room = LabRoom()
    ...
    def get_labroom(db: Session, room_id: int):
    ...
    def get_labrooms(db: Session, skip: int = 0, limit: int = 100):
        return db.query(LabRoom).offset(skip).limit(limit).all()

    def update_labroom(db: Session, room: LabRoom):
    ...
    def delete_labroom(db: Session, room_id: int):
